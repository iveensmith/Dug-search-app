...
// All 774 Local Government Areas, keyed by state (same values as the
// NigerianState enum). Stored as plain strings on Pharmacy.lga — a 774-value
// enum would be unwieldy, and LGA names are stable enough that string
// matching against this canonical list (see isValidLga) is sufficient.
export const LGAS_BY_STATE: Record<NigerianStateValue, string[]> = {
  ABIA: [
...
export function lgasForState(state: string): string[] {
  return LGAS_BY_STATE[state as NigerianStateValue] ?? []
}
