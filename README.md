...
pharmacies manage their own inventory after admin approval; patients can
upload a prescription photo and chat with a licensed pharmacist about it.
...
npm run db:seed       # Akwa Ibom + Lagos pharmacies, 30 drugs, test accounts
...
back to your selected state's capital because the browser blocks location on
insecure origins.
...
| Pharmacy owner (Akwa Ibom) | `mercyland@drugfinder.test` (also: gracecare, uduak, lifespring, firstchoice, vinebranch, rehoboth, citymed, goodness, emem) |
| Pharmacy owner (Lagos) | `lagosmeds@drugfinder.test` (also: ikeja) |
...
zero-result searches); Paracetamol 500 mg is in stock everywhere. Lagos
pharmacies exist specifically to prove state-scoping works — searching from
Akwa Ibom never returns them, and vice versa.

## Trying each feature

- **Patient search** — open `/`, pick a state (auto-detected from your
  location when possible, always overridable), search "panadol" or "flagyl"
  (brand names work), toggle List/Map, tap Directions (draws the route +
  drive time on the in-app map — no Google account needed; a "Voice
  navigation" link hands off to Google Maps for turn-by-turn) or Call.
  Insulin Glargine demonstrates the zero-results state. Routing uses OSRM's
  free demo server; set `OSRM_URL` to a self-hosted instance before real
  launch.
...
- **Pharmacy registration** — `/pharmacy/register`: pick a state, geocode
  button + draggable map pin (biased to the chosen state); new registrations
  land as PENDING.
- **Admin** — log in as `admin@drugfinder.test` → `/admin`: approve/reject
  pharmacies (state shown on each card), add/edit drugs, manage pharmacist
  accounts, and see search-gap analytics.
...
distance search, approval gating, stock-toggle round trip, and state scoping).
...
6. ✅ Nationwide expansion (state-scoped search, was Uyo-only)
