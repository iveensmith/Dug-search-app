import type { DrugForm, NigerianState, VerificationStatus } from '../src/generated/prisma/enums'

// All seed accounts share this password (dev/demo only)
...
export const ORPHAN_DRUG_INDEX = DRUGS.length - 1 // insulin — no stock anywhere
...
export type PharmacySeed = {
  ...
  address: string
  state: NigerianState
  stateAbbr: string // for the fake PCN license number only
  latitude: number
  longitude: number
  phone: string
...
// Fake pharmacies spread across real neighborhoods/roads in two states —
// Akwa Ibom (the original Uyo-only pilot) and Lagos, added specifically to
...
export function inventoryFor(pharmacyIdx: number, drugIdx: number): { stocked: boolean; inStock: boolean } {
